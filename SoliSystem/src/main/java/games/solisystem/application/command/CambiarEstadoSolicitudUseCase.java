...
import games.solisystem.application.dto.CambiarEstadoSolicitudCommand;
...
public class CambiarEstadoSolicitudUseCase {
    ...
    public Solicitud ejecutar(CambiarEstadoSolicitudCommand command) {
        validar(command);
    ...
    private void validar(CambiarEstadoSolicitudCommand command) {
        if (command == null) {
    ...
    private void validarTransicion(EstadoEnum actual, EstadoEnum nuevo) {
        boolean permitida =
                (actual == EstadoEnum.CREADA && nuevo == EstadoEnum.EN_REVISION) ||
